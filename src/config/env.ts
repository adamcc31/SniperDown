...
export const tradingEnv = {
  ...
  get POLYMARKET_SLUG_PREFIX(): string {
    return process.env.POLYMARKET_SLUG_PREFIX || process.env.POLYMARKET_EVENT_SLUG || "btc-updown-5m";
  },
  get BUY_TRIGGER_PRICE(): number {
    return parseNum(process.env.BUY_TRIGGER_PRICE, 0.80);
  },
  /** Don't buy if price is above this (e.g. 0.84). */
  get MAX_BUY_PRICE(): number {
    return parseNum(process.env.MAX_BUY_PRICE, 0.84);
  },
  get STOP_LOSS_PRICE(): number {
    return parseNum(process.env.STOP_LOSS_PRICE, 0.50);
  },
  ...
  get DRY_RUN_MODE(): boolean {
    return process.env.DRY_RUN_MODE !== "false"; // Defaults to true actively if not specified or specified as anything but 'false'
  },
  get TG_BOT_TOKEN(): string {
